...
#include "../inc/MyBOXRun.hh"

MyBOXRun::MyBOXRun(MyBOXDetectorConstruction* det, MyBOXPrimaryGeneratorAction* prim)
:   G4Run(),
    fMyBOXDetector(det),
    fMyBOXPrimary(prim) {
    // init data members
...
void MyBOXRun::Merge(const G4Run* run) {
	...
	const MyBOXRun* localRun = static_cast<const MyBOXRun*>(run);
	//
  ...
  fMyBOXPrimary = localRun->fMyBOXPrimary;
...
void  MyBOXRun::EndOfRunSummary() {
    ...
    primInfo += std::to_string( fMyBOXPrimary->GetParticleEnergy() ) + " [MeV] ";
    primInfo += fMyBOXPrimary->GetParticleName() + "\n";

    G4cout<< "\n\n" << G4endl;
    G4cout<< "\n =================================  Run Summary ================================= \n"
          << " Target thickness    : " << fMyBOXDetector->GetTargetThickness()/CLHEP::cm << " [cm] \n"
          << " Target material info: " << "                                 \n"
          <<  fMyBOXDetector->GetTargetMaterial() << "\n"
