...
MyBOXRunActionMessenger::MyBOXRunActionMessenger(MyBOXRunAction* runAction)
:   G4UImessenger(), 
    fMyBOXRunAction(runAction),
    fDirCMD(nullptr), 
...
MyBOXRunActionMessenger::~MyBOXRunActionMessenger() {
	  delete fEdepHistogramCMD;
    delete fDirCMD;
}


void MyBOXRunActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
...
//        G4cout << fname << " " << minEnergy << " " << maxEnergy << " " << numBins << G4endl;
        fMyBOXRunAction->SetEdepHistogram(fname, minEnergy*unit, maxEnergy*unit,
                                         numBins);
    }
}
